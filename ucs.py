...
def uniform_cost_search(graph, start, goal):
    ...
    while priority_queue:
        ...
        # Print the current state of priority queue and visited nodes
        print(f"\nDequeued node '{current_node}' with cost {current_cost}")
        print("Current priority queue:", priority_queue)
        print("Current visited nodes:", visited)
        ...
        for neighbor, cost in graph[current_node]:
            ...
            if neighbor not in visited or total_cost < visited[neighbor][0]:
                ...
                # Print after enqueuing a new node
                print(f"\nEnqueued neighbor '{neighbor}' with updated cost {total_cost}")
                print("Updated priority queue:", priority_queue)
                print("Updated visited nodes:", visited)
...
if result:
    ...
    print(f"\nLeast cost path from {start_node} to {goal_node}: {' -> '.join(path)} with total cost {total_cost}")
